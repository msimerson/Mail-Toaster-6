...
configure_nginx_server()
{
	...
	store_config "$(get_jail_etc webmail)/nginx/webmail.conf" "overwrite" <<EO_WEBMAIL_INCLUDE
...
{
	local _etc
	_etc="$(get_jail_etc webmail)/nginx"
...
configure_nginx_acme()
{
	...
	local _conf
	_conf="$(get_jail_etc webmail)/nginx/nginx.conf"
	local _acme_conf
	_acme_conf="$(get_jail_etc webmail)/nginx/server.d/acme.conf"
	...
	mkdir -p "$(get_jail_etc webmail)/acme/letsencrypt"
